...
$content = $content -replace '(#define CHIZL_COLORS_FULL_YEAR\s+)\d+', "`${1}$fullYear"
$content = $content -replace '(#define CHIZL_COLORS_YEAR_OFFSET\s+)\d+', "`${1}$yearOffset"
$content = $content -replace '(#define CHIZL_COLORS_MONTH\s+)\d+', "`${1}$month"
$content = $content -replace '(#define CHIZL_COLORS_DAY\s+)\d+', "`${1}$day"
$content = $content -replace '(#define CHIZL_COLORS_UTC_TIME\s+)\d+', "`${1}$time"
